...
const steps: Step[] = [
  ...
  {
    targetId: 'tour-pipeline-controls',
    title: 'Pipeline Controls',
    description: 'Launch or reset the ML pipeline stream here. Fine-tune your configuration on the fly: choose how many transaction samples to process (1, 10, or 100), and set the parallel thread worker pool (1, 2, or 4) for concurrent Python background execution.',
    position: 'bottom'
  },
  {
  ...
  },
  {
    ...
    description: 'Track each transaction from Input through Bronze, Silver, Gold, and Inference layers. View malformed contract violations isolated in the Rejected Stack, and final transaction decisions (including TO_REVISE human reviews) in the Final State.',
    position: 'top'
  },
  {
    targetId: 'tour-logs',
...
function GuidedTourContent() {
  ...
  return (
    <div className="fixed inset-0 z-[9999] pointer-events-none overflow-hidden">
      ...
      {/* Tooltip */}
      <div
        className={`absolute z-10 w-[92vw] max-w-[440px] p-5 lg:p-8 bg-[#1c1e2a] border border-gray-700 rounded-2xl lg:rounded-3xl shadow-2xl pointer-events-auto transition-all duration-500 ease-in-out animate-in zoom-in-95 ${isWelcome ? 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' : ''
          }`}
        style={!isWelcome ? (() => {
